...
type ApiEnvironmentEditRequest struct {
	ctx                   context.Context
	ApiService            *EnvironmentApiService
	id                    string
...
func (r ApiEnvironmentEditRequest) EnvironmentEditAction(environmentEditAction EnvironmentEditAction) ApiEnvironmentEditRequest {
	r.environmentEditAction = &environmentEditAction
	return r
}

func (r ApiEnvironmentEditRequest) Execute() (*EnvironmentItem, *http.Response, error) {
	return r.ApiService.EnvironmentEditExecute(r)
}

/*
EnvironmentEdit Edit an environment.

Edit an environment.

	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@param id Resource identifier
	@return ApiEnvironmentEditRequest
*/
func (a *EnvironmentApiService) EnvironmentEdit(ctx context.Context, id string) ApiEnvironmentEditRequest {
	return ApiEnvironmentEditRequest{
		ApiService: a,
		ctx:        ctx,
		id:         id,
...
//
//	@return EnvironmentItem
func (a *EnvironmentApiService) EnvironmentEditExecute(r ApiEnvironmentEditRequest) (*EnvironmentItem, *http.Response, error) {
	var (
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "EnvironmentApiService.EnvironmentEdit")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}
	...
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.environmentEditAction == nil {
		return localVarReturnValue, nil, reportError("environmentEditAction is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.environmentEditAction
	if r.ctx != nil {
		// API Key Authentication
		if auth, ok := r.ctx.Value(ContextAPIKeys).(map[string]APIKey); ok {
			if apiKey, ok := auth["ApiKeyAuth"]; ok {
